...
items = []
for i in range(0,10):
    items.append(Item("Test" + str(i), (True if i % 2 == 0 else False)))
energy = 100
hours = 4
...
class Hello(FloatLayout):
    ...
    #Main Buttons
        ...
        it = 0
        for item in items:
            self.buttons.append(Button(text = item.name, size_hint=(.3, .05),pos_hint={'x':.1, 'y':.8 - it}))
            it = it + 0.075
        ...
        # self.walk_button = Button(text = testItem.name, size_hint=(.3, .1),pos_hint={'x':.05, 'y':.2})
        print(self.buttons)
